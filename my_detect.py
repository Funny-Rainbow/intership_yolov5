...
Usage - temps:
    $ python detect.py --weights yolov5s.pt --temp 0                               # webcam
                                                     img.jpg                         # image
                                                     vid.mp4                         # video
                                                     screen                          # screenshot
                                                     path/                           # directory
...
def run(
    ...
    is_url = temp.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://'))
    webcam = temp.isnumeric() or temp.endswith('.streams') or (is_url and not is_file)
    screenshot = temp.lower().startswith('screen')
    if is_url and is_file:
        temp = check_file(temp)  # download
    ...
    if webcam:
        ...
        dataset = LoadStreams(temp, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
        bs = len(dataset)
    elif screenshot:
        dataset = LoadScreenshots(temp, img_size=imgsz, stride=stride, auto=pt)
    else:
        dataset = LoadImages(temp, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
